package syscode {
		...
		public class Friends {
				...
				private static function BuildAll() : void {
						...
						Friends.all = [];
						i = 0;
						while(i < Friends.§internal§.length) {
								Friends.all.push(Friends.§internal§[i]);
								i++;
						}
						var x:int = 0;
						while(x < Friends.external.length) {
								b = false;
								i = 0;
								if(i < Friends.§internal§.length) {
										if(Friends.§internal§[i].id == Friends.external[x].id) {
												b = true;
										}
								}
								if(!b) {
