package com.greensock.loading.core {
		...
		public class LoaderCore extends EventDispatcher {
				...
				protected function _errorHandler(event:Event) : void {
						...
						target = event is LoaderEvent && Boolean(this.hasOwnProperty("getChildren")) ? event.target : this;
				...
				protected function _failHandler(event:Event) : void {
						...
						dispatchEvent(new LoaderEvent(LoaderEvent.FAIL,event is LoaderEvent && Boolean(this.hasOwnProperty("getChildren")) ? event.target : this,this.toString() + " > " + (event as Object).text));
				}
