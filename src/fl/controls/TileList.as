...
package fl.controls
{
	...
	public class TileList extends SelectableList implements IFocusManagerComponent
	{
		...
		{
			if (!componentInspectorSetting && _scrollPolicy == value)
			{
				return;
			}
			_scrollPolicy = value;
		...
		override protected function moveSelectionHorizontally(code:uint, shiftKey:Boolean, ctrlKey:Boolean):void
		{
			...
			switch (code)
			{
				case Keyboard.LEFT:
					...
					break;
				case Keyboard.RIGHT:
					index = Math.min(length - 1, selectedIndex + 1);
					break;
				case Keyboard.HOME:
					index = 0;
					break;
				...
				case Keyboard.PAGE_DOWN:
					...
					index = Math.min(length - 1, Math.min(lastIndex, selectedIndex + totalCols));
					break;
			}
